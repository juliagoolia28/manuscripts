...
for i = 1:length(parcel_nums)
    ...
    if num_code(i) ~= 0
        ...
        peak_labels(i) = strcat(hemi,HO_cort_label(round(num_code(i))));
    else
        num_code(i) = HO_sub.vol(peak_vox(i));
        if num_code ~= 0
            peak_labels(i) = HO_sub_label(round(num_code(i)));
        else
            num_code(i) = Cereb.vol(peak_vox(i));
            if num_code ~= 0
                peak_labels(i) = Cereb_label(round(num_code(i)));
            else
